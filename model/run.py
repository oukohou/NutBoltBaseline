...
inference the category using trained models.
"""
...
class ReadImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        # read images
        image_, image_name_ = self.read_images(index)
...
if __name__ == "__main__":
    ...
    model_file = os.path.join(currentpath, model_file)
    image_file_path = sys.argv[1]
    
    inferenced_result = sys.argv[2]
    ...
    label_json = {v: k for k, v in label_json.items()}
    ...
    dst_file.write("id,label\n")
    for index_, (images_, image_names_) in enumerate(inference_data_loader):
        if not use_cpu:
            images_ = images_.to(device)
        outputs = inference_model(images_)
        _, preds = torch.max(outputs, 1)
        results_list.append(preds.tolist()[0])
        for temp_index in range(batch_size):
            single_result_dict['image_name'] = image_names_[temp_index]
            single_result_dict['category'] = label_json[preds.tolist()[temp_index]]
            ...
            dst_file.write("{},{}\n".format(single_result_dict['image_name'].split(".")[0],
                                            str(single_result_dict['category'])))
    ...
    end_time = time.time()
    print("cost {} s, every image {} ms".format(end_time - start_time, (end_time - start_time) / len(os.listdir(image_file_path)) * 1000))
